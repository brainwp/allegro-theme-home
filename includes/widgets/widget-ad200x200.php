<?php
/*
 * Plugin Name: Custom 200x200 Ad Unit
 * Description: A widget that allows the selection and configuration of a single 200x200 Banner
 * Version: 1.0
 ...
 */
add_action( 'widgets_init', 'tz_ad200_widgets' );

/*
 * Register widget.
 */
function tz_ad200_widgets() {
	register_widget( 'TZ_Ad200_Widget' );
}

/*
 * Widget class.
 */
class tz_ad200_widget extends WP_Widget {

	/* ---------------------------- */
	/* -------- Widget setup -------- */
	/* ---------------------------- */
	
	function TZ_Ad200_Widget() {
	
		/* Widget settings */
		$widget_ops = array( 'classname' => 'tz_ad200_widget', 'description' => __('A widget that allows the display and configuration of of a single 200x200 Banner.', THEME_NAME) );

		/* Widget control settings */
		$control_ops = array( 'width' => 200, 'height' => 200, 'id_base' => 'tz_ad200_widget' );

		/* Create the widget */
		$this->WP_Widget( 'tz_ad200_widget', __(THEME_FULL_NAME.' Custom 200x200 Ad', 'framework'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array(
		'title' => '',
		'ad' => get_template_directory_uri()."/images/no-banner-200x200.jpg",
