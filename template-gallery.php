...
<?php get_template_part(THEME_LOOP."loop","start"); ?>
	...
	<div class="block-content">
		...
		<div class="filter-block" id="gallery-categories">
			<strong><?php _e('Filter:', THEME_NAME); ?></strong>
			<a href="#filter" data-option="*" class="active"><?php _e('All', THEME_NAME); ?></a>
					...
					<?php $gallery_style = get_post_meta ( $post->ID, THEME_NAME."_gallery_style", true ); ?>
						...
						<div class="photo-gallery-block gallery-image<?php foreach ($term_list as $term) { echo " ".$term->slug; } ?>" data-id="gallery-<?php the_ID(); ?>">
							...
							<div class="gallery-content">
								...
								?>
								<a href="<?php the_permalink();?>" class="<?php if($gallery_style=="lightbox") { echo 'light-show '; } ?>more" data-id="gallery-<?php the_ID(); ?>"><?php _e("View all photos", THEME_NAME);?>&nbsp;&nbsp;<span class="icon-text">&#9656;</span></a>
							</div>
						</div>
				<?php 
					if ( $paged != 0 ) {
