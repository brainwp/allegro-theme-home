 ...
 * -------------------------------------------------------------------------*/	
	...
	$my_meta->addSelect(THEME_NAME.'_sidebar_position_small',array('right'=>'Right','left'=>'Left'),array('name'=> __('Small Position ','tax-meta'), 'std'=> array('right')));
	$my_meta->addSelect(THEME_NAME.'_sidebar_select_small', $sidebarSmall ,array('name'=> __('Small Sidebar','tax-meta'), 'std'=> array('off')));
	$my_meta->Finish();
 ...
 * -------------------------------------------------------------------------*/
 ...
 function ot_updated_tag_html() {
 	global $post;
 	if (get_the_modified_time() != get_the_time() && get_option(THEME_NAME."_updated_tag")=="on" && get_post_meta ( $post->ID, THEME_NAME."_updated_tag", true )!="no") {
 		echo '<span class="tag" title="'.__("Última atualização: ", THEME_NAME).get_the_modified_time("H:i, j.M Y").'">'.__("Atualizado", THEME_NAME).'</span>';	
 	}
}
...
function orangethemes_comment($comment, $args, $depth) {
	...
	if(!get_avatar_url(get_avatar( $comment, 60))) {
   ...
   ?>
	...
	<li <?php comment_class($comentClass); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="commment-content" id="comment-<?php comment_ID() ?>">
			<?php if(get_avatar_url(get_avatar( $comment, 70))) { ?>
				<div class="user-avatar">
					<a href="<?php if(get_comment_author_url()) { echo get_comment_author_url();} else { echo "#"; } ?>" class="hover-effect">
						<img src="<?php echo get_avatar_url(get_avatar( $comment, 73));?>" class="setborder" alt="<?php printf(__('%1$s', THEME_NAME), get_comment_author());?>" title="<?php printf(__('%1$s', THEME_NAME), get_comment_author());?>" />
					</a>
				</div>
			<?php } ?>
			<strong class="user-nick">
				<a href="<?php if(get_comment_author_url()) { echo get_comment_author_url();} else { echo "#"; } ?>"><?php printf(__('%1$s', THEME_NAME), get_comment_author());?></a>
				<?php if($comment->user_id == get_the_author_meta('ID')) { ?>
					<span class="marker"><?php _e("Author", THEME_NAME);?></span>
				<?php } ?>
			</strong>
		
			<span class="time-stamp"><?php printf(__(' %1$s, %2$s', THEME_NAME), get_comment_date("F j"), get_comment_time("H:i"));?></span>
			<div class="comment-text">
				<?php comment_text(); ?>
			</div>
			<?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth'], 'reply_text' => '<span class="icon-text">&#59154;</span><span>'.( __( 'Reply to this comment' , THEME_NAME )).'<span>'))) ?>
		</div>
<?php
       }
